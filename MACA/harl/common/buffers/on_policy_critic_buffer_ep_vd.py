...
class OnPolicyCriticBufferEPVd(OnPolicyCriticBufferEP):
    """On-policy buffer for critic that uses Environment-Provided (EP) state."""
    ...
    def __init__(self, args, share_obs_space, obs_space, act_space, num_agents):
        ...
        # Buffer for individual agent Q predictions made by this critic.
        # Used for training the actor but not the critic.
        self.ind_q_value_preds = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, num_agents, 1), dtype=np.float32
        )

        # Buffer for individual agent V predictions made by this critic.
        # Used for training the actor but not the critic.
        self.ind_v_value_preds = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, num_agents, 1), dtype=np.float32
        )
    ...
    def insert(
        ...
        self.ind_q_value_preds[self.step] = ind_q_value_preds.copy()
        self.ind_v_value_preds[self.step] = ind_v_value_preds.copy()
