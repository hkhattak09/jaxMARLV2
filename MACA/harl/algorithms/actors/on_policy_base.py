...
from harl.utils.models_tools import update_linear_schedule, RunningMeanStd
...
class OnPolicyBase:
    def __init__(self, args, obs_space, act_space, device=torch.device("cpu")):
        ...
        # create running mean std for policy gradient estimates
        # The variance is inter-batch and inter-state.
        param_vec = torch.cat([p.view(-1) for p in self.actor.parameters() if p.requires_grad], dim=0).detach()
        self.grad_ms = RunningMeanStd(shape=len(param_vec), device=device)
    ...
    def save(self, save_dir, id):
        ...
        torch.save(
            ...
            f"{str(save_dir)}/actor_agent{str(id)}.pt",
        )

    def restore(self, save_dir, id):
        """Restore the actor."""
        self.actor.load_state_dict(
            torch.load(f"{str(save_dir)}/actor_agent{str(id)}.pt")
        )
