    ...
    for player_idx in [0, 1]:
        ...
        if player_type == "manual":
            ...
            raise NotImplementedError(
                "OBL pretrained agents are not vendored in this repository. "
                "Use --player0 manual --player1 manual, or vendor "
                "jaxmarl/environments/hanabi/pretrained first."
            )
        else:
            raise ValueError(
                f"Unsupported player type '{player_type}'. Supported values: manual."
            )
    return agents
