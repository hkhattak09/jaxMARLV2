...
class HanabiEnv(HanabiGame):
    ...
    def __init__(
        ...
        if observation_spaces is None:
            self.observation_spaces = {
                i: Box(low=0.0, high=1.0, shape=(self.obs_size,), dtype=jnp.float32)
    ...
    @partial(jax.jit, static_argnums=[0])
    def get_obs(
        self, new_state: State, old_state: State = None, action: chex.Array = None
    ) -> Dict:
        """Get all agents' observations."""
        if old_state is None:
            old_state = new_state
        if action is None:
            action = self.num_moves - 1  # noop — no prior action on fresh reset
