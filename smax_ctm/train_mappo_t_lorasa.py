"""
MAPPO-T LoRASA fine-tuning script for SMAX.

Copies ``train_mappo_t.py`` and adds unit-type LoRA adapters on top of a
frozen pretrained shared actor backbone, using the existing MACA transformer
critic and advantage computation.
"""
...
from mappo_t import LoRASAActorTrans, TransVCritic, get_default_mappo_t_config
...
def make_train(config):
    ...
    def train(rng, loaded_actor_params, loaded_critic_params, loaded_value_norm_dict):
        ...
        # Set LoRASA config derived from environment
        ...
        if config["LORASA_RANK"] <= 0:
            raise ValueError(f"LORASA_RANK must be positive, got {config['LORASA_RANK']}")
        if config["LORASA_INIT_SCALE"] <= 0:
            raise ValueError(
                "LORASA_INIT_SCALE must be positive so zero-initialized lora_a "
                f"can receive gradients. Got {config['LORASA_INIT_SCALE']}."
            )
        if config["LORASA_NUM_ADAPTER_SLOTS"] <= 0:
            raise ValueError(
                "LoRASA requires at least one unit-type adapter slot; "
                f"env.unit_type_bits={env.unit_type_bits}."
            )
        ...
        actor_params = actor_network.init(
            actor_rng, ac_init_hstate_small, ac_init_x, dummy_adapter_ids
        )

        # Overlay pretrained shared backbone weights, leaving LoRA adapters initialized
        def _overlay_pretrained_actor_params(lorasa_params, pretrained_params):
            lorasa_flat = flatten_dict(lorasa_params)
            pretrained_flat = flatten_dict(pretrained_params)
            for key, value in pretrained_flat.items():
                if key not in lorasa_flat:
                    raise ValueError(
                        f"Pretrained actor param path {key} not found in LoRASA actor params. "
                        "Checkpoint structure mismatch."
                    )
                if value.shape != lorasa_flat[key].shape:
                    raise ValueError(
                        f"Pretrained actor param {key} shape {value.shape} does not "
                        f"match LoRASA backbone shape {lorasa_flat[key].shape}."
                    )
                lorasa_flat[key] = value
            lorasa_keys = set(lorasa_flat.keys())
            pretrained_keys = set(pretrained_flat.keys())
            missing_backbone = {
                k for k in (lorasa_keys - pretrained_keys)
                if not any(part in ("lora_a", "lora_b") for part in k)
            }
            if missing_backbone:
                raise ValueError(
                    f"Backbone leaves missing from pretrained checkpoint: {missing_backbone}"
                )
            return unflatten_dict(lorasa_flat)

        if loaded_actor_params is None:
            raise ValueError("loaded_actor_params is required for LoRASA fine-tuning.")
        actor_params = _overlay_pretrained_actor_params(actor_params, loaded_actor_params)
        ...
        if loaded_critic_params is None:
            raise ValueError("loaded_critic_params is required for LoRASA fine-tuning.")
        # Verify critic structure matches before replacing
        critic_new_flat = flatten_dict(critic_params)
        critic_old_flat = flatten_dict(loaded_critic_params)
        if set(critic_new_flat.keys()) != set(critic_old_flat.keys()):
            raise ValueError(
                "Loaded critic params structure does not match initialized critic. "
                "Keys only in new: "
                f"{set(critic_new_flat.keys()) - set(critic_old_flat.keys())}. "
                "Keys only in old: "
                f"{set(critic_old_flat.keys()) - set(critic_new_flat.keys())}."
            )
        for key, value in critic_old_flat.items():
            if value.shape != critic_new_flat[key].shape:
                raise ValueError(
                    f"Loaded critic param {key} shape {value.shape} does not match "
                    f"initialized critic shape {critic_new_flat[key].shape}."
                )
        critic_params = loaded_critic_params
        # Do NOT apply T-Fixup scaling to loaded critic params
        ...
        # Actor optimizer: update only LoRA adapter parameters
        def _make_lora_mask(params):
            flat = flatten_dict(params)
            mask_flat = {k: ("lora_a" in k or "lora_b" in k) for k in flat}
        ...
        actor_tx = optax.masked(actor_tx, _make_lora_mask(actor_params))
        ...
        if loaded_value_norm_dict is not None:
            value_norm_dict = loaded_value_norm_dict
        elif use_valuenorm:
            print("=" * 80)
            print("WARNING: No pretrained ValueNorm state provided.")
            print("The critic was likely trained with ValueNorm, but we are starting fresh.")
            print("Continuation is NOT exact. Provide --pretrained_valuenorm_path or a")
            print("combined checkpoint containing 'value_norm_dict' for exact continuation.")
            print("=" * 80)
        ...
        def _run_eval(eval_rng, actor_params):
            ...
            def _eval_env_step(carry, _):
                ...
                # Compute adapter ids from current unit types
                unit_types_env = env_s.env_state.state.unit_types[:, :env.num_agents]
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            if pretrained_progress_path is not None:
                ...
                print(
                    ...
                    f"{pretrained_progress_path}; LoRASA logging starts after "
        ...
        def _update_step(update_runner_state, unused):
            ...
            def _env_step(runner_state, step_idx):
                ...
                # Compute unit-type adapter ids from raw SMAX state
            ...
            def _run_actor_epoch(actor_state, rng_epoch):
                ...
                shuffled_adapter_id = _shuffle_reshape(actor_adapter_id)
                if use_recurrent:
                ...
                def scan_actor_minibatch(astate, i):
                    if use_recurrent:
                        mb_data = (
                            ...
                            shuffled_adapter_id[i].swapaxes(0, 1),
...
def _override_config_from_cli(config):
    ...
    parser.add_argument("--pretrained_actor_path", type=str, default=None)
    parser.add_argument("--pretrained_critic_path", type=str, default=None)
    parser.add_argument("--pretrained_valuenorm_path", type=str, default=None)
    ...
    if args.pretrained_checkpoint_path is not None:
        config["PRETRAINED_CHECKPOINT_PATH"] = args.pretrained_checkpoint_path
    if args.pretrained_actor_path is not None:
        config["PRETRAINED_ACTOR_PATH"] = args.pretrained_actor_path
    if args.pretrained_critic_path is not None:
        config["PRETRAINED_CRITIC_PATH"] = args.pretrained_critic_path
    if args.pretrained_valuenorm_path is not None:
        config["PRETRAINED_VALUENORM_PATH"] = args.pretrained_valuenorm_path
    if args.pretrained_progress_path is not None:
        config["PRETRAINED_PROGRESS_PATH"] = args.pretrained_progress_path
    if args.lorasa_rank is not None:
...
def _load_pretrained_state(config):
    """Load pretrained actor, critic, and optional ValueNorm state.

    Prefers a single combined checkpoint. Falls back to separate paths only
    when explicitly provided.
    """
    checkpoint_path = config.get("PRETRAINED_CHECKPOINT_PATH")
    actor_path = config.get("PRETRAINED_ACTOR_PATH")
    critic_path = config.get("PRETRAINED_CRITIC_PATH")
    valuenorm_path = config.get("PRETRAINED_VALUENORM_PATH")
    ...
    if checkpoint_path is not None:
        ...
        if not isinstance(ckpt, dict):
            raise ValueError(
                f"Checkpoint at {checkpoint_path} is not a dict. Got {type(ckpt).__name__}."
            )
        ...
        if loaded_actor_params is None:
            raise ValueError(
                f"Checkpoint at {checkpoint_path} missing 'actor_params'."
            )
        if loaded_critic_params is None:
            raise ValueError(
                f"Checkpoint at {checkpoint_path} missing 'critic_params'."
            )
    else:
        if actor_path is not None:
            if not os.path.isfile(actor_path):
                raise FileNotFoundError(
                    f"--pretrained_actor_path not found: {actor_path}"
                )
            with open(actor_path, "rb") as f:
                loaded_actor_params = pickle.load(f)
            if isinstance(loaded_actor_params, dict) and "actor_params" in loaded_actor_params:
                loaded_actor_params = loaded_actor_params["actor_params"]
        if critic_path is not None:
            if not os.path.isfile(critic_path):
                raise FileNotFoundError(
                    f"--pretrained_critic_path not found: {critic_path}"
                )
            with open(critic_path, "rb") as f:
                loaded_critic_params = pickle.load(f)
            if isinstance(loaded_critic_params, dict) and "critic_params" in loaded_critic_params:
                loaded_critic_params = loaded_critic_params["critic_params"]
        if valuenorm_path is not None:
            if not os.path.isfile(valuenorm_path):
                raise FileNotFoundError(
                    f"--pretrained_valuenorm_path not found: {valuenorm_path}"
                )
            with open(valuenorm_path, "rb") as f:
                loaded_value_norm_dict = pickle.load(f)

    if loaded_actor_params is None:
        raise ValueError(
            "No pretrained actor params loaded. Provide --pretrained_checkpoint_path "
            "or --pretrained_actor_path."
        )
    if loaded_critic_params is None:
        raise ValueError(
            "No pretrained critic params loaded. Provide --pretrained_checkpoint_path "
            "or --pretrained_critic_path."
        )
...
if __name__ == "__main__":
    ...
    # Set LoRASA defaults before any JIT compilation
    config.setdefault("LORASA_RANK", 8)
    config.setdefault("LORASA_INIT_SCALE", 0.01)
    ...
    out = train_jit(rng, loaded_actor_params, loaded_critic_params, loaded_value_norm_dict)
