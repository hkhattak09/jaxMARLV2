...
def make_train(config):
    ...
    def train(rng):
        ...
        ac_init_x = (
            ...
            jnp.zeros((config["NUM_ENVS"], action_dim)),
        )
        ...
        def _update_step(update_runner_state, unused):
            ...
            has_nan = jnp.isnan(total_loss) | jnp.isnan(entropy) | jnp.isnan(returns)
            entropy_low = entropy < 1e-3

            def log_callback(r, s, tl, ent, nan_flag, ent_low):
                line = f"Step {s:8d} | Score: {r:6.2f}/25 | Loss: {tl:10.4f} | Ent: {ent:8.4f}"
                if nan_flag:
            ...
            jax.debug.callback(log_callback, returns, step_count, total_loss, entropy, has_nan, entropy_low)
...
if __name__ == "__main__":
    ...
    print(f"Starting Hanabi MAPPO GRU (num_agents={config['NUM_AGENTS']})...")
    ...
    model_path = os.path.join(model_dir, "hanabi_mappo_gru_actor.pkl")
