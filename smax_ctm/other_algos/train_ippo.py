...
def make_train(config):
    ...
    config["CRITIC_EPOCH"] = config.get("CRITIC_EPOCH", config["UPDATE_EPOCHS"])
    config["ACTOR_NUM_MINI_BATCH"] = config.get("ACTOR_NUM_MINI_BATCH", config["NUM_MINIBATCHES"])
    config["CRITIC_NUM_MINI_BATCH"] = config.get("CRITIC_NUM_MINI_BATCH", config["NUM_MINIBATCHES"])
        ...
        def _update_step(update_runner_state, unused):
            ...
            def _critic_update_epoch(update_state, unused):
                def _critic_update_minbatch(train_states, batch_info):
                    ...
                    if use_recurrent:
                        mb_obs, mb_done, mb_targets, mb_value_old, cr_init_hstate_mb = batch_info
                    else:
                        ac_init_hstate, cr_init_hstate, traj_batch_mb, advantages_mb, targets_mb = batch_info

                    def _critic_loss_fn(critic_params, init_hstate, traj_obs, traj_done, targets, value_old):
                        _, value = critic_network.apply(
                            critic_params, init_hstate, (traj_obs, traj_done),
                        )
                    ...
                    if use_recurrent:
                        ...
                        critic_loss, critic_grads = critic_grad_fn(critic_train_state.params, cr_init_hstate_mb, mb_obs, mb_done, mb_targets, mb_value_old)
                    else:
                        critic_grad_fn = jax.value_and_grad(_critic_loss_fn, has_aux=True)
                        critic_loss, critic_grads = critic_grad_fn(critic_train_state.params, cr_init_hstate, traj_batch_mb.obs, traj_batch_mb.done, targets_mb, traj_batch_mb.value)
