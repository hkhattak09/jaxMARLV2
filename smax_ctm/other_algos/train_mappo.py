"""
MAPPO Baseline for SMAX
Based on smax_ctm/train_mappo_gru.py, using MAPPO config from MACA.
"""
import csv
import os
import sys
...
def make_train(config):
    # Map MACA-style config keys to the keys expected by the GRU trainer loop
    config["FC_DIM_SIZE"] = config["hidden_sizes"][0]
    config["GRU_HIDDEN_DIM"] = config["hidden_sizes"][1]
    config["LR"] = config["lr"]
    config["CRITIC_LR"] = config.get("critic_lr", config["lr"])
    config["UPDATE_EPOCHS"] = config["ppo_epoch"]
    config["CRITIC_EPOCH"] = config["critic_epoch"]
    config["NUM_MINIBATCHES"] = config["actor_num_mini_batch"]
    config["CLIP_EPS"] = config["clip_param"]
    config["ENT_COEF"] = config["entropy_coef"]
    config["VF_COEF"] = config["value_loss_coef"]
    config["MAX_GRAD_NORM"] = config["max_grad_norm"]
    config["GAMMA"] = config["gamma"]
    config["GAE_LAMBDA"] = config["gae_lambda"]
    config["ANNEAL_LR"] = config.get("use_linear_lr_decay", False)
    config["SCALE_CLIP_EPS"] = config.get("scale_clip_eps", False)
    config["OBS_WITH_AGENT_ID"] = config.get("obs_with_agent_id", True)
    config["ACTOR_NUM_MINI_BATCH"] = config.get("ACTOR_NUM_MINI_BATCH", config.get("actor_num_mini_batch", 1))
    config["CRITIC_NUM_MINI_BATCH"] = config.get("CRITIC_NUM_MINI_BATCH", config.get("critic_num_mini_batch", 1))
        ...
        def _update_step(update_runner_state, unused):
            ...
            def _critic_update_epoch(update_state, unused):
                def _critic_update_minbatch(train_states, batch_info):
                    ...
                    if use_recurrent:
                        mb_world_state, mb_done, mb_targets, mb_value_old, cr_init_hstate_mb = batch_info
                    else:
                        ac_init_hstate, cr_init_hstate, traj_batch_mb, advantages_mb, targets_mb = batch_info

                    def _critic_loss_fn(critic_params, init_hstate, traj_world_state, traj_done, targets, value_old):
                        _, value = critic_network.apply(critic_params, init_hstate, (traj_world_state, traj_done))
                        if use_recurrent:
                    ...
                    if use_recurrent:
                        ...
                        critic_loss, critic_grads = critic_grad_fn(critic_train_state.params, cr_init_hstate_mb, mb_world_state, mb_done, mb_targets, mb_value_old)
                    else:
                        critic_grad_fn = jax.value_and_grad(_critic_loss_fn, has_aux=True)
                        critic_loss, critic_grads = critic_grad_fn(critic_train_state.params, cr_init_hstate, traj_batch_mb.world_state, traj_batch_mb.done, targets_mb, traj_batch_mb.value)
...
if __name__ == "__main__":
    ...
    model_path = os.path.join(model_dir, "smax_mappo_actor.pkl")
    ...
    print(f"Saved MAPPO actor checkpoint to {model_path}")
