"""MAPPO-VD Configuration.

The defaults follow the MACA MAPPO-VD paper hyperparameter tables.
"""


def get_default_mappo_vd_config():
    """Return the default MAPPO-VD configuration.
    
    This config combines model, training, and algorithm hyperparameters
    for the MAPPO-VD (Value Decomposition) implementation using SMAX environment.
...
def validate_mappo_vd_config(config, num_agents):
    """Validate MAPPO-VD configuration for minibatch divisibility.
    
    Args:
        config: MAPPO-VD configuration dictionary.
    ...
    if use_naive_recurrent:
        raise NotImplementedError(
            "use_naive_recurrent_policy=True is not implemented in this JAX MAPPO-VD trainer. "
            "Use use_recurrent_policy=True instead."
        )
    if use_recurrent and config.get("recurrent_n", 1) != 1:
        raise NotImplementedError(
            "This JAX MAPPO-VD trainer currently supports recurrent_n=1."
        )
    ...
    if use_recurrent:
        ...
        if data_chunk_length <= 0:
            raise ValueError("DATA_CHUNK_LENGTH must be positive for recurrent MAPPO-VD minibatches.")
        if num_steps % data_chunk_length != 0:
            raise ValueError(
                f"NUM_STEPS({num_steps}) must be divisible by DATA_CHUNK_LENGTH({data_chunk_length}) "
                "for recurrent MAPPO-VD minibatches."
            )
